...
<!-- ![Medallion layers](docs/architecture-medallion.png) -->
...
Landing does no cleaning or casting by design. Keeping the raw copy untouched and auditable, and pushing all transformation downstream, is what makes the source replayable if a transformation rule later turns out to be wrong.
...
The incremental flight load uses `replaceWhere` with `partitionBy` on `batch_id`. Re-running a failed batch replaces only that batch's partition, which makes reprocessing safe and cheap rather than a full table rewrite.
...
**No loop construct.** One pipeline run processes exactly one batch. A monthly schedule trigger fires the next run rather than an Until or ForEach draining the whole backlog in one go.
...
- Orchestration pipeline run in `aeropulse_prod`, processing the two uploaded batches against `flight-data-prod` end to end, landing through to gold.
...
Everything in that table other than the workspace name and the two container and parameter values is **code that is identical between the two**. The notebooks and pipeline definition do not fork or duplicate. Only the environment-specific bindings differ, and those are set by deployment rules rather than by hand.

Fabric's deployment rules can only rebind a notebook's default lakehouse automatically, not arbitrary variables inside it. Turning the ADLS account and container into notebook parameters is what makes them something a rule can target per environment, rather than editing code by hand in every stage.
...
**Least privilege is the organising principle.** Both test users were created with no Entra directory role and no Azure RBAC, so nothing is inherited and every capability they have was granted on purpose. That makes the access model auditable: the answer to "why can this user see this" is always a specific grant, never an accident of inheritance.
...
That middle one is the one usually fudged as "silver is smaller, looks about right". The shrinkage is calculable, so it is checked exactly.

On top of the row counts: surrogate key uniqueness, referential integrity from the fact to every dimension, date keys resolving against the generated calendar, lineage columns surviving into bronze, and every batch appearing in every layer.

Two checks earn their place beyond the obvious. **Dimensions are not empty**, which is the check that would have caught the empty `dim_carrier` that reached Production earlier in this project. And **derived rules still agree with the data they came from**, testing `is_delayed` against the 15 minute arrival delay rule it was built from. Row counts and key integrity both pass happily while business logic has quietly drifted.

### It has to fail loudly

A failed check raises, which fails the notebook. A test that only writes results somewhere needs a person to go and look, and nobody does. The point is that a data correctness problem stops the run the same way a technical exception does.
